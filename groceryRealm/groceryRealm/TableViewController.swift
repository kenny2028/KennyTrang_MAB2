...
class TableViewController: UITableViewController {
    ...
    var groceryDataRunner = GroceryDataHandler()
    var groceryList = [Grocery]()

    override func viewDidLoad() {
        super.viewDidLoad()
        groceryDataRunner.dbSetup() // initializes realm Database
        
        groceryList = groceryDataRunner.getGroceries()// Then update grocery list
    ...
    @IBAction func addPressed(_ sender: UIBarButtonItem) {
        let addalert = UIAlertController(title: "New Item", message: "Add a new item to your grocery list", preferredStyle: .alert)
                ...
                let addItemAction = UIAlertAction(title: "Add", style: .default, handler: {(UIAlertAction)in
                    ...
                    self.groceryDataRunner.addItem(newItem: newGroceryItem)
                    self.reloader()
                })
    ...
    func reloader() {
        groceryList = groceryDataRunner.getGroceries()
    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
            ...
            groceryDataRunner.boughtItem(item: boughtItem)
            reloader()
        }
    ...
    override func tableView(_ tableView: UITableView, commit editingStyle: UITableViewCell.EditingStyle, forRowAt indexPath: IndexPath) {
          if editingStyle == .delete {
              let item = groceryList[indexPath.row]
              groceryDataRunner.deleteItem(item: item)
              reloader()
